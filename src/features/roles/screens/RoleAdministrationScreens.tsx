...
import { roleKeys, useRole, useRolePermissions, useRoles } from '../queries/roleQueries';
...
function RoleFormScreen({ mode }: RoleFormProps) {
  ...
  const remove = useMutation({
    ...
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: roleKeys.lists() }); router.replace('/admin/roles' as Href); },
...
export function RolePermissionsScreen() {
  ...
  const save = useMutation({
    ...
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: roleKeys.permissions }); toast.show({ message: 'Đã cập nhật quyền của vai trò', variant: 'success' }); router.replace(`/admin/roles/${roleCode}` as Href); },
